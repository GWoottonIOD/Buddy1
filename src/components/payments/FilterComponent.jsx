...
export default function FilterComponent(props) {
  ...
  const isPaidOnTheSameDay = (payments) => {
    ...
    payments.forEach(element => {
      ...
      if (!uniqueDates.has(date) || element?.userID !== uniquePayments[uniquePayments?.length - 1]?.userID) {
        uniqueDates.add(date);
        uniquePayments.push(element);
    } else {
      if(!duplicateAdditions.has(date) && element.userID === uniquePayments[uniquePayments.length - 1].userID) {
      duplicatePayments.push(uniquePayments[uniquePayments.length - 1]);
